...
export const HORIZON_PROMPT_VERSION = "fashion-auto-numbering-multi-reference-v11-terra-light-integration";
...
- Treat the MODEL image as the only lighting and color-environment source. Product references provide intrinsic product color, material, construction, fit, and details only; never copy their exposure, white balance, studio light, background color cast, highlights, or shadows.
- Every replaced garment and wearable must be relit as if it was physically present when the MODEL photograph was captured. Match the MODEL image's light direction, intensity, softness, color temperature, exposure, highlight placement, shadow density, ambient bounce light, environmental color cast, and light falloff.
- Make the inherited scene light react correctly to each referenced material. Reconstruct material-appropriate diffuse shading and specular highlights, fabric self-shadowing, folds, tension, body-contact shadows, ambient occlusion, overlap shadows, and edge integration around the neck, shoulders, armpits, sleeves, waist, hips, legs, hems, socks, footwear, and accessories wherever visible.
- The transferred products must never look pasted on, uniformly lit, cut out, floating, haloed, or photographed under a different light. Do not relight or recolor the MODEL person, skin, background, or unchanged regions to accommodate the products; adapt the products to the unchanged MODEL scene instead.
...
export function compilePrompt(s: Record<string, unknown>, ratio: string, images: readonly { category: string }[]): string {
  ...
  const parts: Array<[string, unknown]> = [
    ...
    ["BACKGROUND, COLOR & LIGHTING SOURCE LOCK", "Use the MODEL image as the sole source of truth for the unchanged background, overall color tone, exposure, white balance, light direction, light intensity, softness, color temperature, highlight placement, shadow density, ambient bounce light, environmental color cast, light falloff, floor contact, and cast shadows. Do not relight, recolor, flatten, or otherwise alter the MODEL person, skin, background, or unchanged regions."],
    ["SCENE LIGHT TRANSFER TO EVERY REPLACED PRODUCT", "Relight every authorized garment and wearable so it appears physically present in the MODEL scene at the original moment of capture. Apply the MODEL image's exact illumination consistently across every new product while respecting its referenced material response: diffuse shading, material-appropriate specular highlights, fabric self-shadowing, folds, tension, body-contact shadows, ambient occlusion, overlap shadows, and natural light falloff. Integrate all visible boundaries and contact zones around the neck, shoulders, armpits, sleeves, waist, hips, legs, hems, socks, footwear, and accessories. Product references control intrinsic product color, material, construction, fit, and details only; ignore their photography, exposure, white balance, background color cast, highlights, and shadows."],
    ...
    ["PHYSICAL FIT & MATERIAL REALISM", `${String(s.fit_material_realism ?? "").trim()} Render physically accurate garment construction, fabric drape, folds, tension, contact points, occlusion, perspective, and shadows on the unchanged MODEL body and pose. The product must conform to the body volume and inherit the local scene illumination at every surface instead of carrying flat or reference-photo lighting. Match the reference hem-to-footwear geometry exactly: long trousers may stack over and obscure shoe uppers, while shorts or cropped trousers must retain their referenced exposure.`.trim()],
    ...
    ["DO NOT ADD, CHANGE, OR IMPORT", "Do not import any reference person, body, pose, camera, crop, background, lighting, exposure, white balance, shadows, or unrelated item. Do not preserve any original base garment or wearable whose role is authorized for replacement. Reject pasted-on clothing, flat garment lighting, mismatched light direction or color temperature, inconsistent highlights or shadows, floating fabric, missing contact shadows, cutout or halo edges, and any artificial compositing appearance. No extra people, duplicated products, text, watermark, interface, collage, or split image."],
    ["FINAL OUTPUT", `One centered subject only. ${ratio} aspect ratio. One continuous undivided photorealistic image. The result must look exactly like the MODEL photograph was originally captured with every authorized product already being worn in that same physical lighting environment, never like clothing was pasted, overlaid, or composited afterward. Every authorized product must be visibly replaced and no authorized original garment may remain. Product replacement has absolute priority over clothing preservation; only non-clothing MODEL attributes and explicitly unreferenced wearable roles remain unchanged.`],
  ];
