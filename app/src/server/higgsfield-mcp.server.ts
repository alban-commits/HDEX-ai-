...
function searchModelMatches(
  ...
  return (
    candidate.id === target.providerModelId ||
...
function buildModelProfile(input: {
  ...
  const canonicalIdentity = id === input.target.providerModelId;
...
export async function inspectHiggsfieldProvider(input: {
  ...
  for (const target of MODEL_TARGETS) {
      ...
      canonicalDetail =
        detailRecord(
          await input.callTool("models_explore", {
            action: "get",
            model_id: target.providerModelId,
          }),
    ...
    if (!profile.available) {
      // Search and list remain bounded diagnostics only and never select an execution model.
      try {
