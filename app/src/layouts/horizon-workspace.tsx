...
import { ensureHorizonCompletedDirectory, HorizonDirectoryScanError, pickHorizonBatchDirectory, requestHorizonBatchDirectoryPermission, restoreHorizonBatchDirectory, saveHorizonBatchNative, scanHorizonDirectory, supportsHorizonDirectoryPicker, type HorizonDirectoryHandle, type HorizonDirectoryPermission } from "@/lib/horizon-filesystem.browser";
...
type BatchState = HorizonBatchJob & { status: HorizonBatchStatus; message?: string; results?: HorizonBatchDownload[]; savedFiles?: string[]; savedResultCount?: number; saveFailureCount?: number; successCount?: number; failureCount?: number };
...
export function HorizonWorkspace({ onBack, onParentWorkspaceReset, parentBusy = false }: { onBack: () => void; onParentWorkspaceReset: () => void; parentBusy?: boolean }) {
  ...
  const scanSelectedDirectory = async (handle: HorizonDirectoryHandle) => {
    ...
    try {
      await ensureHorizonCompletedDirectory(handle);
      applyBatchScan(await scanHorizonDirectory(handle));
      setMessage("선택한 원본 경로에 완성본 폴더를 준비하고 상품 이미지를 확인했습니다.");
    } catch (error) {
  ...
  const chooseBatchDirectory = async () => {
    if (!supportsHorizonDirectoryPicker()) {
      ...
      setBatchScanError("선택한 경로에 결과 파일을 직접 저장할 수 있는 Chrome 또는 Edge에서 열어 주세요.");
    ...
    try {
      ...
      setMessage("폴더 읽기·쓰기 권한을 확인하지 못했습니다. 같은 원본 폴더를 다시 선택하고 권한을 승인해 주세요.");
    }
  };
  const rescanBatchDirectory = async () => {
    if (!batchDirectory) { await chooseBatchDirectory(); return; }
    try {
      ...
      else setMessage("선택한 원본 경로에 완성본을 저장하려면 읽기·쓰기 권한을 승인해야 합니다.");
    } catch {
  ...
  const approveBatchDirectory = async () => {
    ...
    try {
      ...
      if (permission === "granted") await scanSelectedDirectory(batchDirectory);
        ...
        setMessage("폴더 권한이 없어 작업을 시작할 수 없습니다. 원본 폴더의 읽기·쓰기 권한을 승인해 주세요.");
      }
    } catch {
      setMessage("폴더 권한을 승인하지 못했습니다. 원본 폴더를 다시 선택해 주세요.");
    }
  };
  const startBatch = async () => {
    ...
    if (!batchDirectory || batchDirectoryPermission !== "granted") {
      setMessage("선택한 원본 경로의 읽기·쓰기 권한을 승인해야 일괄 생성을 시작할 수 있습니다.");
      return;
    }
        ...
        try {
          ...
          await withHorizonUploadedAssets(current.files.map((entry) => entry.file), async (assets) => {
            ...
            if (outcome.results.length) {
              try {
                saved = await saveHorizonBatchNative({ root: batchDirectory, results: outcome.results });
              } catch {
                saved = { savedFiles: [], savedResultCount: 0, failureCount: outcome.results.length, failedResults: [...outcome.results] };
              }
            }
            const persistenceComplete = saved.savedResultCount === outcome.results.length;
            setBatch((items) => items.map((item) => item.key === current.key ? {
              ...
              message: outcome.successCount > 0 && !persistenceComplete
                ? `이미지 ${outcome.successCount}장은 생성됐지만 선택한 원본 경로의 완성본 폴더에 결과 파일을 저장하지 못했습니다.`
                : outcome.successCount > 0
                ? `${outcome.successCount}장 성공${outcome.failureCount ? ` · ${outcome.failureCount}장 생성 실패` : ""}${saved.savedResultCount ? ` · 선택 경로의 완성본에 원본 형식 ${saved.savedResultCount}장 저장` : ""}${saved.failureCount ? ` · ${saved.failureCount}장 저장 실패` : ""}`
                : "완료된 이미지 결과가 없습니다.",
            } : item));
          });
        } catch (error) {
  ...
  const saveBatchResultsAgain = async (results: readonly HorizonBatchDownload[]) => {
    if (!batchDirectory || batchDirectoryPermission !== "granted") {
      setMessage("선택한 원본 경로의 읽기·쓰기 권한을 다시 승인해 주세요.");
      return;
    }
    const saved = await saveHorizonBatchNative({ root: batchDirectory, results });
    setMessage(saved.failureCount
      ? `${saved.savedResultCount}장의 결과 파일은 선택 경로의 완성본에 저장했고 ${saved.failureCount}장은 저장하지 못했습니다.`
      : `${saved.savedResultCount}장의 결과 파일을 원본 형식 그대로 선택 경로의 완성본에 저장했습니다.`);
  };
  const saveAllBatchResultsAgain = async () => {
    ...
    for (const item of savable) {
      const saved = await saveHorizonBatchNative({ root: batchDirectory, results: item.results ?? [] });
      successCount += saved.savedResultCount;
      failureCount += saved.failureCount;
    }
    setMessage(failureCount
      ? `${successCount}장의 결과 파일은 선택 경로의 완성본에 저장했고 ${failureCount}장은 저장하지 못했습니다.`
      : `${successCount}장의 결과 파일을 원본 형식 그대로 선택 경로의 완성본에 저장했습니다.`);
  };
