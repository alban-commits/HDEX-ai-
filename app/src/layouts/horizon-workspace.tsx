...
import { downloadHorizonLayeredPsd } from "@/lib/horizon-restore.browser";
...
function HorizonRecentGeneration({ item, psdBusy, onRestore }: { item: HorizonGalleryItem; psdBusy: boolean; onRestore: (item: HorizonGalleryItem) => void }) {
  ...
  return <div className="hz-recent-result"><GenerationTile ratio="square" src={item.src} alt={item.alt} className="hz-recent-tile" generation={{ src: source, ...(item.kind === "video" ? { mediaType: "video" as const, poster: item.src } : { mediaType: "image" as const }), aspectRatio: item.width / item.height, prompt: item.prompt }} actions={[downloadAction]} detail={{ actions: [downloadAction] }} openLabel={`원본 결과 보기: ${item.prompt}`} />{item.kind === "image" ? <button type="button" className="hz-restore-open" disabled={psdBusy} onClick={() => onRestore(item)}>{psdBusy ? "PSD 만드는 중…" : "원본+생성본 PSD 바로 받기"}</button> : null}</div>;
}
...
export function HorizonWorkspace({ onBack, onParentWorkspaceReset, parentBusy = false }: { onBack: () => void; onParentWorkspaceReset: () => void; parentBusy?: boolean }) {
  ...
  const downloadLayeredPsd = async (item: HorizonGalleryItem) => {
    ...
    setMessage("1번 원본과 생성본을 같은 크기의 2레이어 PSD로 만들고 있습니다.");
    let ownedOriginalUrl: string | undefined;
    try {
      const originalUrl = original.file
        ? (ownedOriginalUrl = URL.createObjectURL(original.file))
        : original.src;
      await downloadHorizonLayeredPsd({
        originalUrl,
        generatedUrl: `/api/higgsfield/result/${encodeURIComponent(item.id)}`,
        filename: `horizon-${item.id}`,
      });
      setMessage("원본 모델과 AI 생성본이 겹쳐진 2레이어 PSD를 저장했습니다.");
    } catch {
      setMessage("레이어 PSD를 만들지 못했습니다. 원본 이미지와 생성 결과를 다시 확인해 주세요.");
    } finally {
