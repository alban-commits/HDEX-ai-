...
import { flattenFeedPages, jobsFeedQueryOptions, prependGenerations, removeGenerationQueries, useFnfJobClient, useFnfScopeKey, useGenerationRun, useLiveFeedGenerations } from "@higgsfield/fnf-react";
...
function HorizonRecentGeneration({ item }: { item: HorizonGalleryItem }) {
  ...
  return <GenerationTile ratio="square" src={item.src} alt={item.alt} className="hz-recent-tile" generation={{ src: source, ...(item.kind === "video" ? { mediaType: "video" as const, poster: item.src } : { mediaType: "image" as const }), aspectRatio: item.width / item.height, prompt: item.prompt }} actions={[downloadAction]} detail={{ actions: [downloadAction] }} openLabel={`원본 결과 보기: ${item.prompt}`} />;
}
...
export function HorizonWorkspace({ onBack, onParentWorkspaceReset, parentBusy = false }: { onBack: () => void; onParentWorkspaceReset: () => void; parentBusy?: boolean }) {
  ...
  useEffect(() => { const fresh = run.generations.filter((generation) => !created.current.has(generation.id)); if (!fresh.length) return; for (const item of fresh) created.current.add(item.id); prependGenerations(queryClient, HISTORY_QUERY, fresh, { scopeKey }); }, [queryClient, run.generations, scopeKey]);
    ...
    try {
      const done = await runHorizonGenerationFlow({
        ...
        generate: async (resolvedCommand) => {
          ...
          return run.start(buildGenerationInput(engine, resolvedCommand, ratio, quantity, selected));
        },
      });
      if (!done) return;
  ...
  const clearCurrentBrowserWorkspace = () => {
    ...
    created.current.clear(); run.reset();
    removeGenerationQueries(queryClient, { scopeKey });
  };
  const reset = () => {
    if (parentBusy || run.isRunning || batchRunning || batchActive.current || generationActive.current || promptBusy || promptFlight.current || uploadReservations.current > 0) { setMessage("처리 중에는 전체 초기화를 할 수 없습니다."); return; }
    clearCurrentBrowserWorkspace(); setMessage("작업 내용을 초기화했습니다.");
  };
  const leaveHorizon = () => {
    if (parentBusy || run.isRunning || batchRunning || batchActive.current || generationActive.current || promptBusy || promptFlight.current || uploadReservations.current > 0) { setMessage("처리 중에는 화면을 이동할 수 없습니다."); return; }
    onBack();
  };
  ...
  const chooseFolder = (files: ArrayLike<File>) => setBatch(scanHorizonFolder(boundedHorizonFolderFiles(files)).map((job) => ({ ...job, status: "queued" })));
  const startBatch = async () => {
    if (parentBusy || batchActive.current || generationActive.current || run.isRunning || !requireSignIn()) return; batchActive.current = true; setBatchRunning(true);
      ...
      for (const current of batch) {
        ...
        try {
          await withHorizonUploadedAssets(current.files.map((entry) => entry.file), async (assets) => {
            ...
            for (const generation of done) created.current.add(generation.id);
            prependGenerations(queryClient, HISTORY_QUERY, done, { scopeKey });
            ...
            setBatch((items) => items.map((item) => item.key === current.key ? {
              ...
              message: outcome.successCount > 0
                ? `${outcome.successCount}장 성공${outcome.failureCount ? ` · ${outcome.failureCount}장 실패` : ""}`
                : "완료된 이미지 결과가 없습니다.",
            } : item));
          });
