...
import { downloadHorizonPngAndLayeredPsd } from "@/lib/horizon-restore.browser";
...
type BatchState = HorizonBatchJob & { status: HorizonBatchStatus; message?: string; results?: HorizonBatchDownload[]; originalFile?: File; savedFiles?: string[]; savedResultCount?: number; saveFailureCount?: number; successCount?: number; failureCount?: number };
...
export function HorizonWorkspace({ onBack, onParentWorkspaceReset, parentBusy = false }: { onBack: () => void; onParentWorkspaceReset: () => void; parentBusy?: boolean }) {
  ...
  const saveBatchResultsAgain = async (results: readonly HorizonBatchDownload[], originalFile?: File) => {
    if (!batchDirectory || batchDirectoryPermission !== "granted") {
    ...
    const saved = await saveHorizonBatchPngPsd({ root: batchDirectory, original: originalFile, results });
  ...
  const saveAllBatchResultsAgain = async () => {
    ...
    for (const item of savable) {
      const saved = await saveHorizonBatchPngPsd({ root: batchDirectory, original: item.originalFile!, results: item.results ?? [] });
