...
import { HorizonDirectoryScanError, pickHorizonBatchDirectory, requestHorizonBatchDirectoryPermission, restoreHorizonBatchDirectory, saveHorizonBatchPngPsd, scanHorizonDirectory, supportsHorizonDirectoryPicker, type HorizonDirectoryHandle, type HorizonDirectoryPermission } from "@/lib/horizon-filesystem.browser";
import { HORIZON_HISTORY_QUERY as HISTORY_QUERY, syncHorizonHistory } from "@/lib/horizon-history";
import { createHorizonPngPsdArtifacts, downloadHorizonPngAndLayeredPsd, downloadHorizonPngPsdArtifacts } from "@/lib/horizon-restore.browser";
...
type BatchState = HorizonBatchJob & { status: HorizonBatchStatus; message?: string; results?: HorizonBatchDownload[]; originalFile?: File; savedFiles?: string[]; savedResultCount?: number; saveFailureCount?: number; browserDownloadCount?: number; successCount?: number; failureCount?: number };
...
export function HorizonWorkspace({ onBack, onParentWorkspaceReset, parentBusy = false }: { onBack: () => void; onParentWorkspaceReset: () => void; parentBusy?: boolean }) {
  ...
  const downloadBatchPngPsd = async (
    ...
    for (const result of results) {
      try {
        const artifacts = await createHorizonPngPsdArtifacts({
          original: originalFile,
          generated: result.url,
          filename: result.filename,
        });
        downloadHorizonPngPsdArtifacts(artifacts);
        successCount += 1;
      } catch {
        failureCount += 1;
      }
    ...
    if (announce) {
      setMessage(failureCount
        ? `${successCount}개 결과의 PNG+PSD 다운로드를 시작했고 ${failureCount}개는 실패했습니다.`
        : `${successCount}개 결과의 PNG+PSD 다운로드를 시작했습니다.`);
    }
  ...
  const startBatch = async () => {
    ...
    try {
      await runHorizonBatchSequence(batch, async (current) => {
        ...
        try {
          const originalFile = current.files.find((entry) => HORIZON_SLOTS.find((slot) => slot.id === entry.slotId)?.group === "model")?.file;
          if (!originalFile) throw new HorizonBatchStepError("uploading", "1번 원본 모델 이미지를 찾지 못했습니다.");
          await withHorizonUploadedAssets(current.files.map((entry) => entry.file), async (assets) => {
            ...
            if (batchDirectory && batchDirectoryPermission === "granted" && outcome.results.length) {
              try {
                saved = await saveHorizonBatchPngPsd({ root: batchDirectory, original: originalFile, results: outcome.results });
              } catch {
                saved = { savedFiles: [], savedResultCount: 0, failureCount: outcome.results.length, failedResults: [...outcome.results] };
              }
            }
            const browserResults = batchDirectory && batchDirectoryPermission === "granted"
              ? saved.failedResults
              : outcome.results;
  ...
  const downloadBatchResults = async (results: readonly HorizonBatchDownload[], originalFile?: File) => {
    if (!originalFile) {
      setMessage("PNG+PSD를 만들 1번 원본 모델 파일이 없습니다. 폴더를 다시 선택해 주세요.");
      return;
    }
    await downloadBatchPngPsd(results, originalFile);
  };
  const downloadAllBatchResults = async () => {
    const downloadable = batch.filter((item) => item.originalFile && item.results?.length);
    let successCount = 0;
    let failureCount = 0;
    for (const item of downloadable) {
      const downloaded = await downloadBatchPngPsd(item.results ?? [], item.originalFile!, false);
      successCount += downloaded.successCount;
      failureCount += downloaded.failureCount;
    }
    setMessage(failureCount
      ? `${successCount}개 결과의 PNG+PSD 다운로드를 시작했고 ${failureCount}개는 실패했습니다.`
      : `${successCount}개 결과의 PNG+PSD 다운로드를 시작했습니다.`);
