...
interface TemplatePickerModalCommonProps {
...
/** Live pickers require product templates; seeded cards are explicit demo mode. */
export type TemplatePickerModalProps = TemplatePickerModalCommonProps &
  ({ demo: true; items?: never } | { demo?: false; items: TemplateItem[] });

export function TemplatePickerModal(props: TemplatePickerModalProps) {
  const { trigger, onSelect, defaultOpen } = props;
  const [category, setCategory] = useState("all");
  const [kind, setKind] = useState("all");
  const items = props.demo ? TEMPLATES : props.items;
