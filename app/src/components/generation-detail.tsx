 ...
 * Live callers should pass the actions they support; omitted live actions stay
 * hidden rather than rendering non-functional demo buttons.
 */

export interface GenerationDetail {
  /** Durable media source for the generated image or video. */
  src: string;
...
interface GenerationDetailModalCommonProps {
   ...
   * from the fields present on `generation`.
   */
  detailRows?: GenerationDetailRow[];
  /**
   * The full-width lime CTA. Omit or pass `null` to hide it in live mode.
   * Explicit demo mode supplies the visual-preview default.
   */
  primaryAction?: GenerationDetailPrimaryAction | null;
  /**
   * The footer action buttons below the CTA. Omit or pass `[]` to hide the row
   * in live mode. Explicit demo mode supplies visual-preview defaults.
   */
...
/** Live viewers require real generation data; demo fallback is explicit. */
...
function InfoPanel({
  ...
  // Fixed layout, configurable content. Live mode derives only real metadata
  // rows and renders only actions the caller wires.
  ...
  const primary =
    primaryAction === undefined ? (demo ? DEFAULT_PRIMARY_ACTION : null) : primaryAction;
  const footerActions = (actions ?? (demo ? DEFAULT_ACTIONS : [])).map((action) =>
